$(document).ready(function() {
    ...
    $('#modal-filter-list-agenda-cliente').on('shown.bs.modal', function() {
        initializeAutocomplete();
    });
    ...
    $('#modal-new-agenda-cliente').on('shown.bs.modal', function() {
        initializeAutocomplete();
    });
    ...
    $('#dado_cliente_desc').on('input', function() {
        var inputVal = $(this).val();
        if(inputVal.length >= 2) {
            $.ajax({
                url: '/inicio/autocomplete/cliente-by-desc',
                type: 'GET',
                data: { 'desc': inputVal },
                success: function(data) {
                    ...
                    data.forEach(function(item) {
                        $('<div>').text(item.descricao)
                                .appendTo(autocompleteList)
                                .click(function() {
                                    $('#dado_cliente_desc').val(item.descricao);
                                    $('#id_cliente').val(item.codigo);
                                    autocompleteList.empty();
                                });
                    });
                }
